...
import { X, Search, Loader2, Save, User, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export default function GlobalSettingsModal({ isOpen, onClose, adminProjects }) {
  ...
  // { [projectId]: { assigned: boolean, role: string, initialAssigned: boolean, initialRole: string, memberId: string | null } }
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      // Fetch their project memberships
      ...
      // Initialize assignments state based on adminProjects
      const newAssignments = {};
      adminProjects.forEach(proj => {
        const mem = memberships?.find(m => m.project_id === proj.id);
        newAssignments[proj.id] = {
          assigned: !!mem,
          role: mem ? mem.role : 'pm', // default to PM if not assigned
          initialAssigned: !!mem,
  ...
  const handleSave = async () => {
    ...
    for (const proj of adminProjects) {
      ...
      // If changed from not assigned to assigned -> INSERT
      ...
      // If changed from assigned to not assigned -> DELETE
      ...
      // If stayed assigned but role changed -> UPDATE
    ...
    try {
      ...
      // Force cache invalidation so if they click a project, the team list is fresh
      ...
      // Re-fetch the data to true-up the UI state
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm transition-opacity" onClick={onClose}>
      <div className="w-full max-w-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-slate-800/30 shrink-0">
          <form onSubmit={handleSearch} className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl pl-9 pr-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-sky-500"
              />
            </div>
            <button
              type="submit"
              disabled={isSearching || !searchEmail.trim()}
              className="bg-slate-900 hover:bg-slate-800 text-white dark:bg-white dark:text-slate-900 dark:hover:bg-slate-200 px-5 py-2.5 rounded-xl text-sm font-bold transition-colors disabled:opacity-50 flex items-center gap-2 shadow-sm"
            >
              {isSearching ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Lookup'}
            </button>
          </form>
          {searchError && <p className="text-red-500 text-xs font-semibold mt-2">{searchError}</p>}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 custom-scrollbar bg-white dark:bg-slate-900">
          {!targetUser ? (
             <div className="h-full flex flex-col items-center justify-center text-slate-400 p-8 text-center">
                ...
                <p className="text-sm font-medium">Search for a user to manage their project assignments.</p>
                <p className="text-xs mt-1 max-w-xs text-balance">You can assign them to any project where you currently hold an Admin role.</p>
             </div>
          ) : (
            <div className="space-y-4">
              <div className="flex items-center gap-3 p-3 bg-sky-50 dark:bg-sky-900/10 border border-sky-100 dark:border-sky-900/50 rounded-xl">
                ...
                <div>
                  <div className="font-bold text-sm text-slate-900 dark:text-white">{targetUser.display_name || 'Unknown User'}</div>
              ...
              <div className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
                <div className="bg-slate-50 dark:bg-slate-800 px-4 py-2 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center text-xs font-bold text-slate-500 uppercase tracking-wider">
                  <span>Project</span>
                  <span>Role Assignment</span>
                </div>
                <div className="divide-y divide-slate-100 dark:divide-slate-800">
                  {adminProjects.length === 0 ? (
                    <div className="p-6 text-center text-sm text-slate-500">You do not have Admin access to any projects.</div>
                  ) : (
                    adminProjects.map(proj => {
                      ...
                      return (
                        <div key={proj.id} className="p-3 px-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                          ...
                          <select
                            ...
                            className={`pl-2 pr-6 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider outline-none cursor-pointer border-r-4 border-transparent ${
        ...
        <div className="p-5 border-t border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-slate-800/30 flex items-center justify-between shrink-0">
           <div className="flex-1">
             {saveStatus.message && (
               <div className={`flex items-center gap-1.5 text-xs font-bold ${saveStatus.type === 'success' ? 'text-emerald-500' : saveStatus.type === 'error' ? 'text-red-500' : 'text-slate-500'}`}>
           ...
           <div className="flex items-center gap-3">
             <button
              ...
              <button
                ...
                className="bg-sky-500 hover:bg-sky-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-colors disabled:opacity-50 flex items-center gap-2 shadow-sm"
              >
