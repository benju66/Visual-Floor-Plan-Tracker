...
export default function StampPreview({
  ...
  if (!sourceUnit || !sourceUnit.polygon_coordinates || (sourceUnit.polygon_coordinates as PercentPoint[]).length === 0) return null;
  ...
  let sumX = 0, sumY = 0;
  const polyCoords = sourceUnit.polygon_coordinates as PercentPoint[];
  polyCoords.forEach(pt => { sumX += pt.pctX; sumY += pt.pctY; });
  const cx = sumX / polyCoords.length;
  const cy = sumY / polyCoords.length;
  const dx = pctX - cx;
  const dy = pctY - cy;

  const translatedPoints = polyCoords.map(pt => ({
    pctX: pt.pctX + dx,
    pctY: pt.pctY + dy
  }));
