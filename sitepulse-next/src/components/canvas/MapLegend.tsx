    ...
    <>
      ...
      >
        ...
        {activeTemporalStates.length > 0 && (
          <Group y={padding + milestonesHeight + (activeMilestones.length > 0 ? padding : 0)} listening={false}>
            <Text
              x={padding}
              y={0}
              text="Map Statuses"
            ...
            {activeTemporalStates.map((state, idx) => {
              ...
              const iconColor = TEMPORAL_COLORS[state as string] || '#cbd5e1';
              return (
                <Group key={state as string} y={itemY}>
                  <Group x={padding + 7} y={7} scale={{ x: 0.8, y: 0.8 }}>
                    ...
                    <Path
                      x={-8}
                      y={-8}
                      data={(ICON_PATHS as any)[state as TemporalState] || ICON_PATHS.completed}
                      fill="transparent"
                      stroke={iconColor}
                      strokeWidth={2}
                      strokeLineCap="round"
                  ...
                  <Text
                    x={padding + 22}
                    y={0}
                    text={(state as string).charAt(0).toUpperCase() + (state as string).slice(1)}
                    fontSize={14}
                    fill="#475569"
                    verticalAlign="middle"
                    height={14}
