...
### Phase 6 — Workbench tracing: reuse the canvas to bank labels  ✅ SHIPPED 2026-06-17
...
- **As shipped (branch `claude/workbench-phase6-tracing`):**
  - **New tracing sub-route** `src/app/workbench/[sheetId]/page.tsx` (a page, not a modal — it hosts a zoom/pan canvas) that resolves the drawing via the container-scoped `useWorkbenchSheets` (so only a real workbench sheet can open) and renders `WorkbenchTracer`.
  - **`src/components/workbench/WorkbenchTracer.tsx`** — mounts the REUSED, unchanged `FloorplanCanvas` (`activeStatuses`/`rawStatuses` = `[]`, no `applicabilityIndex`); points the shared `useMapStore.activeSheetId` at the workbench sheet on mount and **restores the prior live-map session (sheet/tool/selection) on unmount** so nothing leaks into the live map. Wires `onPolygonComplete` → the reused `UnitNamingPopover` + `TaxonomyPicker`, fed the **sheet's** `sheet_project_type`. + **`WorkbenchTracerToolbar.tsx`** (Pan/Trace/Select + snapping only — no status/lag/legend/route/bulk).
  - **`useCreateWorkbenchLabel(sheetId)`** (`src/hooks/useWorkbenchActions.ts`) — a slim wrapper that reuses the live `useCreateUnit(sheetId)` + `taxonomyResultToUnitFields`/`useProposePendingSubtype`, sets `unit_type`/`top_level_role`/`subtype_id`/`computed_area`, and carries the **write-site `kind='workbench'` guard**. Area via the pure unit-tested `computeLabelArea` (`src/utils/workbench.ts`).
  - **Tracing floating state** added to `useWorkbenchStore` (pending polygon / popover-open / draft name) — kept separate from the live map's `useMapStore`/`useUIStore` so the two never share mutable popover state.
  - **Container read-hook hardening (the Open-decision follow-up):** `useWorkbenchContainer` dropped `staleTime: Infinity` + validates `kind`, so a stale/poisoned IDB-persisted container **self-heals on next visit** (reads self-correct, not just the write-site guard).
  - **Backward-compatible guard** in `CanvasContextMenu` + `ContextActionDock`: status/milestone/history (and unsupported edit) buttons render only when their handler is supplied. The live map passes them all (unchanged); the workbench passes none, so a selected label exposes only geometry actions — **no status control anywhere in the tracer**.
  - **No `main.py`/RLS/migration/service-role changes.** `FloorplanCanvas`, the PDF worker, snapping engine, taxonomy pickers, and `progressAnalytics` are untouched.
  - **Verified live (real backend not required — PDF render is client-side, vectors were cached):** opened the Phase-5 test drawing → canvas mounts, picker scoped to the sheet's Healthcare type; traced + saved 3 labels (`unit_type`=sub-type, `top_level_role`, `subtype_id`, `computed_area`, 4 pts), which **persist across reload**, write **zero `status_logs`**, leave `spans_levels`/`has_void` NULL, and never appear on the dashboard. The `Other (pending)` UI branch wasn't drivable via automation (proposer local-state vs. synthetic events) but reuses the identical, already-proven live resolver.

### Phase 7 — Standard-enforcing labeling UX (A4): naming rules + label metadata + Definition-of-Done/review  ✅ SHIPPED 2026-06-18
- **Scope:**
...
- **As shipped (merged to `main` `a97f0aa`, 2026-06-18 — built in one session, NOT split 7a/7b):**
  - **`src/utils/workbenchNaming.ts`** (pure, unit-tested — `workbenchNaming.test.ts`, 19 cases): `normalizeLocationName` (trim + collapse spaces), `isNameUniqueOnSheet` (normalized + case-insensitive), `suggestNextName` (auto-increment the dominant designator series — numeric / prefixed / trailing-number, padding-preserving, null on no-pattern), `definitionOfDoneChecks` (the §9 checklist: ≥1 label, all named, names trimmed, names unique, all carry a `top_level_role`).
  - **Naming popover is a NEW workbench-only component** `src/components/workbench/WorkbenchLabelPopover.tsx` (NOT extra props on the shared live `UnitNamingPopover.jsx`, so the live map's contract is untouched): requires a role + sub-type before save, **hard-blocks** within-sheet duplicates with a one-click `suggestNextName` fix, trims on save, and adds the `spans_levels`/`level_note` + `has_void` toggles and interior-face guidance. It **doubles as the in-place EDIT popover** (pre-fills type/flags from the unit).
  - **Editing (added mid-phase on owner request):** clicking a label on the canvas exposes **Rename + Delete** — the tracer now passes `onRenameUnit`/`onDeleteUnit` to the reused `FloorplanCanvas` (the Phase-6 handler-presence gate makes those buttons appear without surfacing any status/milestone action). Plus a **review table** (`WorkbenchReviewTable.tsx`) to rename / re-type / toggle flags / delete every label inline, gated by the live DoD checklist — opened from the header review badge (`WorkbenchReviewControl.tsx`).
  - **Review lifecycle:** `review_state` on `workbench_sheets` (`draft → ready_for_review → reviewed`), stamping `reviewed_by`/`reviewed_at`; **"Mark reviewed" is disabled until `definitionOfDoneChecks(...).passed`**. State + transitions live in the review table footer.
  - **Hooks** (`src/hooks/useWorkbenchActions.ts`): `useUpdateWorkbenchReviewState` (carries the `kind='workbench'` write-site guard via the shared `assertWorkbenchContainer`) + `useUpdateWorkbenchLabel` (normalize + taxonomy resolve + flag writes; reuses the live `useDeleteUnit` for deletes). Label create now writes `spans_levels`/`level_note`/`has_void`. Review-state presentation centralized in `workbench.ts` (`REVIEW_STATES`, `REVIEW_STATE_LABELS`/`_BADGE`, `narrowReviewState`).
  - **No migration** (Phase 3 already shipped every column), **no `main.py`/RLS/service-role changes.** New floating state (`editingLabelId`) added to `useWorkbenchStore` with the explicit interface.
  - **Verified:** typecheck + 303 tests + build green; owner click-through of naming rules, metadata toggles, DoD/review, canvas rename/delete, and the review table on the live test drawing.
...
- **Duplicate-name handling** (Phase 7) — **RESOLVED & SHIPPED (Phase 7)**: **hard-block** within-sheet name collisions (standard §4.5), with `suggestNextName` offered as the one-click fix — in both the create/edit popover and the review table (owner-confirmed 2026-06-18).
- **Container query robustness** (Phase 6/7 follow-up — surfaced in Phase 5) — **RESOLVED & SHIPPED (Phase 6)**: `useWorkbenchContainer` dropped `staleTime: Infinity` and now validates `kind` in its queryFn, so a persisted value is revalidated-on-mount via the route and a stale/poisoned/wrong-kind container **self-heals on the next visit** — reads self-correct, alongside the Phase-5 write-site guard (now mirrored in `useCreateWorkbenchLabel`).
