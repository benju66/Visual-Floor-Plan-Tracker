...
- **Scope:**
  ...
  - **✅ AUDIT RESULT (2026-07-15, prod `pmccdxmuszuykawvlphj`):** `project_members` holds only `admin` (8) + `pm` (2) — **0 `'super'` and 0 `'sub'` rows.** The backfill is therefore a **no-op**; on the owner's decision (2026-07-15) it was **NOT run and NO migration was created** — the audit itself is the record that the data is already clean. The app-code sweep (which stops future `'super'` writes at the source + self-heals any stray value via `normalizeLegacyRole`) is the real fix. A DB `CHECK`/enum guard on the free-text `role` column is the durable belt-and-suspenders — deferred as a schema change out of this phase's surgical scope (and to avoid tangling with the future Subcontractor RLS buildout).
