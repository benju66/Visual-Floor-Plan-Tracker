...
- **Reclaiming storage blobs on delete** → deferred (own follow-up brief, NOT a Phase-8 sub-phase). Discovered closing Phase 8c (2026-06-18): `storage.objects` has RLS **enabled with zero policies**, so the client `supabase.storage.from('floorplans').remove([...])` is default-denied project-wide — both the workbench hard-delete AND the live app's `handleDeleteSheet` orphan the `converted/<id>.png` + `originals/<id>.pdf` files (the DB rows are correctly destroyed; only the blobs linger). Fixing it needs a **backend delete endpoint** (`main.py`, service-role storage delete — the same role that uploads them) or a **storage RLS DELETE policy** — both were out of scope for 8c (no backend / no RLS widening). The right fix is a tiny authenticated backend route that service-role-deletes a sheet's storage objects, called by both delete paths. Until then, blobs accumulate harmlessly (keyed by dead sheet UUIDs, unreachable from the app) and can be swept from the Supabase Storage dashboard.
