...
- **Map Lag Mode** (`mapSettings.colorByVariance`, toggled from `MapHorizontalToolbar`) recolors unit polygons by schedule variance. The recoloring happens **inside `FloorplanCanvas`** on copies of the bottleneck statuses (`displayStatuses`) — never recolor `mapDisplayStatuses` in `page.jsx`, because that array feeds write paths (`BulkActionDock` bottlenecks, quick modals) and variance colors would be written to `status_logs.status_color`.
