...
* **Slot-Unique Status Model:** `status_logs` enforces `UNIQUE(unit_id, activity_id)` — one current-state row per slot, keyed by the activity's stable id (formerly the milestone name; see `20260701_activity_model.sql`). All mutations use the `upsert_status_log` RPC or `.upsert()`. History is preserved in the append-only `status_audit_log` table via a Postgres trigger.
...
| `20260701_activity_model.sql` | **⚠️ Destructive (rename + re-key) — Scheduling Foundation Slice A, Phase 1.** Renames `project_milestones` → `activities` (+ `type` `'task'`/`'milestone'`); re-keys `status_logs` from the mutable `milestone` TEXT name to `activity_id uuid → activities(id)` (`ON DELETE CASCADE`), swapping the slot constraint to `UNIQUE(unit_id, activity_id)` and updating the `upsert_status_log` RPC (still `SECURITY INVOKER` + LWW guard, no `anon` grant) and the audit trigger; adds `status_audit_log.activity_id` (`ON DELETE SET NULL`) while **keeping** `milestone` as a point-in-time name snapshot; renames `milestone_applicability_overrides` → `activity_applicability_overrides` (`milestone_id` → `activity_id`) and `sheets.milestone_schedules` → `activity_schedules`. Name→id conversions (matched on `project_id, track, name`) were dry-run against prod as 100% lossless (2301 status + 1799 audit rows, 0 unmatched). Guarded/idempotent; apply in lockstep with the matching frontend deploy. Back up / confirm PITR first |
...
| `20260702_activity_dictionary.sql` | **Additive + nullable (not deploy-coupled) — Scheduling Foundation Slice A, Phase 2.** Creates the global governed `activity_dictionary` table (cross-project canonical activities — globally-unique `name`, governed `status` active/pending/deprecated, `type` task/milestone, optional default `track` hint, JSONB `aliases` + `default_project_types`, reserved `cost_code_id` for Slice B), **mirroring the `subtypes` dictionary** column-for-column and RLS-for-RLS (read = any member, write = `owner`/`admin`/`pm`, never `anon`); seeds the `Other (pending)` sentinel + the distinct existing activity names (globally-unique `name` collapses a name spanning tracks — e.g. `Framing` — to one entry); and adds `activities.dictionary_id uuid → activity_dictionary(id) ON DELETE SET NULL` (nullable — existing activities start NULL = the review queue). Additive only; the Phase-1 status/sync pipeline is untouched and the live app ignores the new table/column until the activity editor reads it. Dated `20260702` so it replays after `20260701_activity_model.sql` (which it depends on). Idempotent; safe to re-run |
| `20260703_activity_dependencies.sql` | **Additive + isolated (empty on arrival) — Scheduling Foundation Slice A, Phase 3b.** Creates the `activity_dependencies` table: light Finish-to-Start edges between a project's activities (`predecessor_activity_id`/`successor_activity_id` FKs → `activities(id)` `ON DELETE CASCADE`, `type` pinned to `'FS'` by CHECK, `lag_days INT` — may be negative for a lead, pair-`UNIQUE`, self-dependency CHECK). **Coarse by design** — no CPM/float math anywhere; the v1 Schedule-view UI authors one predecessor per activity (the pair-unique table allows a DAG later). No `project_id` column — project scope flows through the FKs; RLS read = any member of the successor's project, write = `owner`/`admin`/`pm` (INSERT additionally requires both endpoints in the same project), never `anon`. Touches no existing table/RPC/RLS. Idempotent; safe to re-run |
| `20260710_schedule_baselines.sql` | **Additive + isolated (empty on arrival) — Unified Schedule Engine, Phase 4.** Creates the `schedule_baselines` table: named, **immutable** whole-project snapshots of the schedule PLAN (JSONB `snapshot` = both layers: each sheet's level×activity windows + every dated slot's planned window; **never** progress fields), so a re-import can be diffed against the plan-of-record and accepted change-by-change. Append-only by design — RLS has **no UPDATE policy** (read = any member, INSERT/DELETE = `owner`/`admin`/`pm`, never `anon`); multiple named baselines per project (no UNIQUE), newest-first index `(project_id, created_at DESC)`. Touches no existing table/RPC/RLS (only FKs `projects(id)` `ON DELETE CASCADE`). Idempotent; safe to re-run |
| `20260712_project_dates.sql` | **Additive + nullable (no backfill) — Band vs Promise, Phase 1.** Adds `projects.construction_start_date` and `projects.contract_completion_date` (both `date NULL`, ISO `'YYYY-MM-DD'`) — two owner-entered project dates, where the contract completion date is the "promised finish" the dashboard's confidence band is measured against in Phase 2. Entered in the new Settings → Project Info tab. **No RLS change** — the existing row-level `projects` UPDATE policy (owner/admin) governs the new columns (verified against prod: it's a per-row membership check, not a column allow-list). Phase 1 stores + displays only; nothing renders on the dashboard yet. Idempotent; safe to re-run |
| `20260706_audit_activity_name.sql` | **Metadata-only rename (append-only history) — Terminology Sweep follow-up.** Renames the last data-layer `milestone` remnant `status_audit_log.milestone` → `activity_name` (the point-in-time activity-name snapshot the audit trigger writes on every status change) and re-keys `fn_status_log_audit` to write the new column name — the function body is otherwise byte-for-byte unchanged (still `SECURITY DEFINER` + `search_path`, still looks up the activity's current name from `activities`). Instant column RENAME (no row rewrite); the table's two indexes key on `(unit_id, track)` / `(unit_id, temporal_state, logged_date)` and don't reference the column; `fn_status_log_audit` is the only DB object that touches it (verified — no view, no other function). No `status_logs` / `upsert_status_log` / RLS / offline-queue change. Apply in lockstep with the matching frontend deploy (the `useUnitHistory` / `useStatusHistory` read hooks select `activity_name` after this). Guarded/idempotent; safe to re-run |
| `20260712_status_logs_preserve_omitted_fields.sql` | **Non-destructive RPC replace — Status Sequencing & Data-Integrity Fix, Phase 5 (FINAL).** `CREATE OR REPLACE`s `upsert_status_log` so an OMITTED JSON key now **preserves** the stored column instead of wiping it: `log_data ? 'field'` false (absent) → keep existing; present-but-empty (`''`/null) → clear to NULL; present value → set. Only the `DO UPDATE SET` expressions for `planned_start_date` / `planned_end_date` / `logged_date` / `actual_start_date` / `status_color` change (each wrapped in `CASE WHEN log_data ? 'field' …`); the INSERT/VALUES, the LWW `client_timestamp` guard, `ON CONFLICT (unit_id, activity_id)`, `temporal_state`/`track`/`client_timestamp` writes, and posture are byte-for-byte — **(implicit-default) `SECURITY INVOKER`, same `search_path`, grants unchanged, `anon` never granted**. The DB backstop against any current/future caller silently NULLing a column by leaving it out. Shipped with caller reconciliations (`useUpdateStatus` stops dropping a null `logged_date`; `useClearStatus` sends explicit-empty color+dates for a full "Not Started" reset; the auto-advance side-write sends explicit-null completion/actual-start). Touches no existing row. Idempotent; safe to re-run |
