...
| `20260619_units_standard_version.sql` | **Additive + nullable (no backfill).** Labeling-standard version stamping (Fill-From-Walls plan, Phase V): adds nullable `units.standard_version TEXT` so every label records which version of `docs/location-labeling-standard.md` it was created under (standard §10 — keeps the training corpus internally consistent as the rulebook evolves). Workbench label creation stamps it from `LABELING_STANDARD_VERSION` in `src/utils/locationTaxonomy.ts`; existing rows stay NULL (honest — they predate stamping). **RLS unchanged, no `anon` grant**; the status/sync pipeline and live app are untouched. Idempotent; safe to re-run |
