...
> **Feasibility findings (2026-06, tested on 3 real sheets — projects A/B/C):**
> - **PDFs are clean vector, not scans** (50k–168k vector objects/sheet; embedded rasters are only logos/detail bubbles). ✅
> - **No OCG layers in any sheet** — layer-based wall isolation is out. Walls must be found by lineweight/color/geometry.
> - **Wall isolation works, but the signature differs per project:** C = heavy+black; B = heavy, *any* color (gray walls); A = *medium* black (heavy = grid/callouts), and A's walls share weight+color with its dimension lines (attributes alone can't separate them). → **Hand-tuned geometric rules do not generalize across projects** — the empirical case for the learned model (M3).
> - **Room-closing is the real bottleneck (M2.2).** Morphological close on the isolated skeleton recovered the corridor + some interior rooms but **perimeter units leak out through window/door gaps (~16 of ~40 rooms, parameter-sensitive)**. Robustifying it (footprint sealing, opening detection, double-line centerlines) is multi-week and stays brittle. **Decision: do NOT pursue geometric auto-detect as the bootstrap.**
> - **Label extraction is clean and free** (M2.3): room names + unit numbers come out as whole strings ("UNIT A1", "KITCHEN", "CORRIDOR", "4001"…) at consistent font sizes on all three sheets. ✅
> - **Chosen bootstrap:** manual tracing on the existing snapping engine **+ free auto-naming from the text layer** → capture → train M3 (the real auto-detector). M2.2 deferred; M2.5 (SAM) optional spike if interactive per-room assist is wanted later.
>
> **Feasibility findings round 2 (2026-06, 3 *commercial* sheets — Aldi grocery / LaSalle office-TI / Crew carwash):**
> - **Still 100% clean vector, no scans** (now 6/6 sheets). OCR/scan path stays low priority.
> - **OCG layers are NOT universally absent — the round-1 "layers are out" conclusion was too broad.** Aldi carries the full **AIA CAD layer standard**: separate named layers for walls (`A-Wall-Stud-Int`, `A-Wall-Finish`, `A-Wall-Exist`), **doors** (`A-Door`, `A-Door-Jamb`), **windows/glazing** (`A-Glaz`), **columns** (`S-Cols`), and the grid. Verified: `page.get_drawings(extended=True)` tags **every** path with its `layer`, so per-layer vector extraction works. **When layers exist this is the cleanest possible extraction** (walls/doors/windows/columns free + machine-exact training labels) and it **revives room-closing** (walls + door layer → seal gaps at known doors). LaSalle + Crew expose **no** layers (like A/B/C) → opportunistic/firm-dependent, not universal.
> - **Naming-from-text holds for commercial**, with a wrinkle: spaces carry **name + space number** ("417 WOMEN", "OFFICE 110") plus **separate door numbers** ("105A") tied to a door schedule. Naming tool must capture name+number and not confuse door tags (font size separates them).
> - **Architect/firm is reliably in the title-block legal notice** (LaSalle → "…written permission of **RSP Architects**"). But **title-block position varies by firm** (Aldi/Crew differ from LaSalle) → confirms the human box-drag tool over auto-location.
> - **Wall signatures still differ per layerless sheet** (LaSalle/Crew both 0.24pt-dominant, different wall weights) → per-set calibration / model still needed for the layerless majority.
> - **Commercial sheets reference wall-type legends + door schedules** (Crew: "SEE WALL TYPE LEGEND") → wall types are explicitly coded on-plan, making future wall-type capture more tractable for commercial.
...
2. **Bootstrap assist (revised per 2026-06 feasibility test):** **M2.1** text-layer extraction → **M2.3** auto-naming on manual trace (name + type fill themselves when a room is drawn over its label). This rides the existing snapping engine; the user traces, the app names. **M2.2 geometric room-closing is deferred** (room-closing proved the brittle bottleneck). **M2.5 SAM** is an optional later spike if interactive per-room segmentation is wanted. This bootstrap generates clean training data fastest with least risk.
